...
function stampBuildMetadata(opts) {
  ...
  const script = [
    ...
    `BUILD_INSTALLED_AT=${shellQuote(new Date().toISOString())}`,
    `tmp="$(mktemp)"`,
    `src="$BUILD_FILE"`,
    `if [ ! -f "$src" ]; then src="$(mktemp)"; printf '{}\\n' > "$src"; fi`,
    `jq --arg version "$BUILD_VERSION" --arg commit "$BUILD_COMMIT" --arg branch "$BUILD_BRANCH" --arg channel "$BUILD_CHANNEL" --arg repo "$BUILD_REPO" --arg installedAt "$BUILD_INSTALLED_AT" '. + {version: $version, commit: $commit, branch: $branch, channel: $channel, repo: $repo, installedAt: $installedAt}' "$src" > "$tmp"`,
    `mv "$tmp" "$BUILD_FILE"`,
    `if [ "$src" != "$BUILD_FILE" ]; then rm -f "$src"; fi`,
  ].join('\n');
  run(command('ssh'), [
    '-o', 'BatchMode=yes',
    '-o', 'ConnectTimeout=8',
