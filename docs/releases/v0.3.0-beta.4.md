# Project: Caroline v0.3.0-beta.4

Public beta platform validation release.
...
- Raspberry Pi OS Desktop 64-bit kiosk mode is validated as the primary beta kiosk path.
- Steam Deck / SteamOS is smoke-validated on nightly/server-client mode by stable LAN IP.
- Ubuntu Desktop VM server-client mode is validated with a 50GB disk, CPU-only Ollama, and `qwen2.5:1.5b`; beta smoke passes once the VM is reachable on the LAN.
- Phone and tablet browsers are lightweight LAN clients for quick chat, status, settings, and compact controls.
...
- Added a Settings > System Check panel that runs read-only host smoke checks for backend health, realtime, AI runtime, memory shards, host resources, updates, and enabled integrations.
- Added a first-pass phone/tablet browser layout that keeps the kiosk desktop view intact while focusing small screens on avatar presence, chat, status, settings, and compact controls.
...
- Expanded the README and beta docs to explain Caroline as a host/kiosk that can be reached from phone browsers, LAN desktop browsers, or the desktop companion app.
- Bumped the companion app version and download links to `0.1.12`.
- Documented Ubuntu Desktop VM server/client validation with 50GB disk, CPU-only Ollama, and `qwen2.5:1.5b`.
- Marked Raspberry Pi OS Desktop as a validated primary beta kiosk platform and Steam Deck / SteamOS as smoke-validated on nightly.
...
- Replaced the first-run workstyle questions with funnier personality prompts that capture warning labels, derail patterns, nudge style, overwhelm support, accountability, day thieves, and victory types.
...
- Added automatic Raspberry Pi kiosk performance mode that preserves widgets while reducing decorative animation and low-priority browser polling.
- Stopped hidden avatar GIFs and disabled video streams from decoding in the background on kiosk displays.
- Bypassed the Raspberry Pi Chromium wrapper for Caroline launchers when the real Chromium binary is available, avoiding an obsolete injected V8 flag.
...
- Fixed SteamOS nightly updates when older local beta tags need to be refreshed from GitHub.
- Fixed SteamOS updates so an existing LAN bind setting is preserved across reinstall/update runs.
...
- Documented the `companion-v0.1.12` fix for repeated raw **Task list updated.** and **Calendar updated.** messages in companion transcripts.
...
- Fixed question-shaped memory saves so relationship-memory questions normalize to the fact before later recall.
...
- Ubuntu Desktop VM was re-smoked after restoring LAN access and updating the installed beta metadata to `v0.3.0-beta.4`.
