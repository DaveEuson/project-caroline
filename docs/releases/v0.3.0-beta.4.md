...
- Ubuntu Desktop VM server-client mode remains validated with a 50GB disk, CPU-only Ollama, and `qwen2.5:1.5b`; rerun smoke after confirming the VM is reachable on the LAN.
...
- Ubuntu VM smoke could not be rerun during this cut because the VM dropped off the LAN during validation.
