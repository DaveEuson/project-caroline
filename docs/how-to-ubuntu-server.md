...
If `/tmp/caroline-node-apt.log` says `nodejs : Conflicts: npm`, rerun the installer from the latest `release`. NodeSource bundles npm inside its `nodejs` package, so older installers that ask apt for both packages can confuse Ubuntu's dependency solver.
